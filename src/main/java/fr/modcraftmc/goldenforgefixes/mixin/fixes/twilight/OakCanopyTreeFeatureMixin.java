...
@Mixin(value = OakCanopyTreeFeature.class, remap = false)
public class OakCanopyTreeFeatureMixin {
    ...
    protected boolean generate(WorldGenLevel world, RandomSource random, BlockPos pos, BiConsumer<BlockPos, BlockState> trunkPlacer, BiConsumer<BlockPos, BlockState> leavesPlacer, BiConsumer<BlockPos, BlockState> decorationPlacer, TFTreeFeatureConfig config) {
        // determine a height
        final List<BlockPos> leaves = Lists.newArrayList();
        ...
        buildTrunk(world, trunkPlacer, random, pos, treeHeight, config, leaves);
        ...
        for (int b = 0; b < numBranches; b++) {
            ...
            buildBranch(world, pos, trunkPlacer, treeHeight - 10 + (b / 2), 5, bangle, btilt, false, random, config, leaves);
    ...
    private void buildTrunk(LevelAccessor world, BiConsumer<BlockPos, BlockState> trunkPlacer, RandomSource rand, BlockPos pos, int treeHeight, TFTreeFeatureConfig config, List<BlockPos> leaves) {
     ...
     */
    void buildBranch(LevelAccessor world, BlockPos pos, BiConsumer<BlockPos, BlockState> trunkPlacer, int height, double length, double angle, double tilt, boolean trunk, RandomSource treeRNG, TFTreeFeatureConfig config, List<BlockPos> leaves) {
